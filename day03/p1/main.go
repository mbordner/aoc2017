...
func main() {
	fmt.Println(getDistanceFromCenter(277678))
}

func getDistanceFromCenter(n int) int {
	if n == 1 {
		return 0
	}
	level, squares, minVal, _ := getLevel(n)
	distance := (n - minVal) % (squares / 4)
	return level + common.Abs(distance-(level-1))
}

// getLevel returns level number, number of squares at this level, min num and max num
func getLevel(n int) (int, int, int, int) {
	...
	return level, squares, minVal, maxVal
}
